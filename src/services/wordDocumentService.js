...
const { t } = require('../locales/wordTranslations');
 ...
 */
function buildIdentificationSection(data, reportName, isServiceProvider, language = 'pt') {
  const clientLabel = isServiceProvider ? t(language, 'client') : t(language, 'company');
  ...
  return [
    ...
    createHeading(t(language, 'identification'), HeadingLevel.HEADING_2),
    createInfoRow(clientLabel, data.client?.name),
    createInfoRow(t(language, 'address'), data.client?.address),
    createInfoRow(t(language, 'contact'), data.client?.contact),
 ...
 * Build Scope section
 */
function buildScopeSection(data, language = 'pt') {
  const sections = [
    createHeading(t(language, 'scope'), HeadingLevel.HEADING_2),
    createParagraph(
 ...
 */
function buildSystemsSection(data, includePhotos, systemPhotoBuffers = new Map(), language = 'pt') {
  const sections = [
    createHeading(t(language, 'systems'), HeadingLevel.HEADING_2)
  ];

  if (!data.systems || data.systems.length === 0) {
    sections.push(createParagraph(t(language, 'noSystems')));
    return sections;
  }
  ...
  if (systemsWithChildren.length > 0) {
    sections.push(createHeading(t(language, 'processStages'), HeadingLevel.HEADING_3));
  ...
  if (includePhotos && systemPhotoBuffers.size > 0) {
    sections.push(createHeading(t(language, 'systemPhotos'), HeadingLevel.HEADING_3));
 ...
 * @param {string} language - Language code
 */
function buildAnalysesSection(data, block, chartBuffers = { field: [], laboratory: [] }, language = 'pt') {
  const sections = [];
  ...
  if (showFieldOverview) {
    sections.push(createHeading(t(language, 'fieldMonitoringOverview'), HeadingLevel.HEADING_2));
    if (fieldLogs.length === 0) {
      sections.push(createParagraph(t(language, 'noFieldData')));
    } else {
      const headers = [
        t(language, 'date'),
        t(language, 'system'),
      ...
      if (block.highlightAlerts) headers.push(t(language, 'alerts'));
  ...
  if (includeCharts && chartBuffers.field && chartBuffers.field.length > 0) {
    sections.push(createHeading(t(language, 'fieldMonitoringCharts'), HeadingLevel.HEADING_3));
  ...
  if (showFieldDetailed && fieldLogs.length > 0) {
    sections.push(createHeading(t(language, 'fieldMonitoringDetailed'), HeadingLevel.HEADING_2));
    sections.push(...buildDetailedAnalysisTable(fieldLogs, language));
  }

  // Laboratory Overview
  if (showLaboratoryOverview) {
    sections.push(createHeading(t(language, 'laboratoryMonitoringOverview'), HeadingLevel.HEADING_2));
    if (laboratoryLogs.length === 0) {
      sections.push(createParagraph(t(language, 'noLaboratoryData')));
    } else {
      const headers = [
        t(language, 'date'),
        t(language, 'system'),
  ...
  if (includeCharts && chartBuffers.laboratory && chartBuffers.laboratory.length > 0) {
    sections.push(createHeading(t(language, 'laboratoryMonitoringCharts'), HeadingLevel.HEADING_3));
  ...
  if (showLaboratoryDetailed && laboratoryLogs.length > 0) {
    sections.push(createHeading(t(language, 'laboratoryMonitoringDetailed'), HeadingLevel.HEADING_2));
    sections.push(...buildDetailedAnalysisTable(laboratoryLogs, language));
  }
 ...
 */
function buildDetailedAnalysisTable(logs, language = 'pt') {
  const sections = [];
  const MAX_DAYS = 7;
  ...
  if (uniqueDates.length === 0) {
    sections.push(createParagraph(t(language, 'noDetailedData')));
    return sections;
  }
  ...
  const headers = [
    ...
    t(language, 'range'),
    ...uniqueDates.map(d => formatShortDate(d))
  ];
 ...
 */
function buildInspectionsSection(data, block, language = 'pt') {
  const sections = [
    createHeading(t(language, 'inspections'), HeadingLevel.HEADING_2)
  ];

  if (!data.inspections || data.inspections.length === 0) {
    sections.push(createParagraph(t(language, 'noInspections')));
    return sections;
  }
  ...
  if (displayInspections.length === 0) {
    sections.push(createParagraph(
      highlightOnlyNC ? t(language, 'noInspectionsWithNC') : t(language, 'noInspections')
    ));
    return sections;
  }
  ...
  // Overview
  if (showOverview) {
    sections.push(createHeading(t(language, 'inspectionsOverview'), HeadingLevel.HEADING_3));

    const headers = [
      t(language, 'date'),
      t(language, 'system'),
      t(language, 'inspector'),
      t(language, 'status'),
      t(language, 'compliant'),
      t(language, 'nonCompliant')
    ];
  ...
  // Detailed
  if (showDetailed) {
    sections.push(createHeading(t(language, 'inspectionsDetailed'), HeadingLevel.HEADING_3));
    ...
    displayInspections.forEach(insp => {
      ...
      if (insp.conclusion) {
        sections.push(createParagraph(`${t(language, 'conclusion')}: ${insp.conclusion}`));
      }
    });
  }
 ...
 */
function buildOccurrencesSection(data, block, language = 'pt') {
  const sections = [
    createHeading(t(language, 'occurrences'), HeadingLevel.HEADING_2)
  ];

  if (!data.incidents || data.incidents.length === 0) {
    sections.push(createParagraph(t(language, 'noOccurrences')));
    return sections;
  }
  ...
  if (filteredIncidents.length === 0) {
    sections.push(createParagraph(t(language, 'noOccurrencesFiltered')));
    return sections;
  }

  // Overview
  if (showOverview) {
    sections.push(createHeading(t(language, 'occurrencesOverview'), HeadingLevel.HEADING_3));
    sections.push(createParagraph(
      t(language, 'showingOccurrences', {
  ...
  if (block.includeTimeline && filteredIncidents.length > 0) {
    sections.push(createHeading(t(language, 'timeline'), HeadingLevel.HEADING_3));
  ...
  // Detailed
  if (showDetailed) {
    sections.push(createHeading(t(language, 'occurrencesDetailed'), HeadingLevel.HEADING_3));
    ...
    filteredIncidents.forEach(inc => {
      ...
      if (block.includeComments && inc.comments && inc.comments.length > 0) {
        sections.push(new Paragraph({
          children: [new TextRun({ text: `${t(language, 'comments')} (${inc.comments.length}):`, bold: true })],
 ...
 */
function buildConclusionSection(data, language = 'pt') {
  const sections = [
    createHeading(t(language, 'conclusion'), HeadingLevel.HEADING_2)
  ];

  // Summary statistics
  sections.push(createHeading(t(language, 'reportSummary'), HeadingLevel.HEADING_3));

  const summary = data.summary || {};
  sections.push(createInfoRow(t(language, 'totalSystems'), String(summary.totalSystems || 0)));
  sections.push(createInfoRow(t(language, 'totalAnalyses'), String(summary.totalReadings || 0)));
  sections.push(createInfoRow(t(language, 'totalInspections'), String(summary.totalInspections || 0)));
  sections.push(createInfoRow(t(language, 'totalOccurrences'), String(summary.totalIncidents || 0)));

  // Alerts
  if (summary.outOfRangeCount > 0) {
    sections.push(new Paragraph({
      children: [
        new TextRun({ text: `⚠ ${t(language, 'alertsFound')}: `, bold: true, color: 'DC2626' }),
        new TextRun({ text: t(language, 'outOfRangeReadings', { count: summary.outOfRangeCount }), color: 'DC2626' })
      ],
      spacing: { before: 150, after: 80 }
    }));
  }
  ...
  if (summary.openIncidents > 0) {
    sections.push(new Paragraph({
      children: [
        new TextRun({ text: `⚡ ${t(language, 'openIncidents')}: `, bold: true, color: 'F59E0B' }),
  ...
  // Conclusion text
  if (data.conclusion) {
    sections.push(createHeading(t(language, 'conclusionText'), HeadingLevel.HEADING_3));
    sections.push(new Paragraph({
    ...
    sections.push(new Paragraph({
      children: [new TextRun({ text: t(language, 'noConclusionProvided'), italics: true, color: '666666' })],
      spacing: { after: 200 }
    }));
  }
 ...
 */
async function generateWordDocument(reportData, config, reportName, templateLogo = null, chartData = null, language = 'pt') {
  ...
  for (const block of enabledBlocks) {
    switch (block.type) {
      case 'identification':
        children.push(...buildIdentificationSection(reportData, reportName, isServiceProvider, language));
        break;
      case 'scope':
        children.push(...buildScopeSection(reportData, language));
        break;
      case 'systems':
        children.push(...buildSystemsSection(reportData, block.includePhotos, systemPhotoBuffers, language));
        break;
      case 'analyses':
        children.push(...buildAnalysesSection(reportData, block, chartBuffers, language));
        break;
      case 'inspections':
        children.push(...buildInspectionsSection(reportData, block, language));
        break;
      case 'occurrences':
        children.push(...buildOccurrencesSection(reportData, block, language));
        break;
      case 'conclusion':
      ...
      case 'attachments':
        ...
        children.push(createParagraph(t(language, 'attachmentsDescription')));
        break;
    }
  }
